...
from statsmodels.tsa.stattools import OLS, add_constant, adfuller
...
async def get_beta(ctx, asset_a, asset_b="", period="", resolution_minutes="", dollar_size="100$"):
    if ctx.channel.id == config['channel_ids']['commands_test']:
        if asset_a == 'help':
            help_msg = 'Usage:\n You want to check the beta between BTC and ETH based on last 10 days of 5min data and calculate how much units of each based on 2500$ of BTC \n Example:\n`!beta btc-perp eth-perp 30days 5min 2500$`'
            help_msg += '\nMethodology: Uses OLS Linear Regression. Tries both permutations and returns the beta of the combination that has the lowest pvalue of ADFULLER test'
            await ctx.channel.send(help_msg)
        else:
            period = int(period.replace('days', ''))
            resolution_minutes = int(resolution_minutes.replace('min', ''))
            dollar_size = float(dollar_size.replace('$', ''))
            ...
            if period > max_period_allowed:
                ...
                try:
                    ...
                    corr = data_a_returns.corr(data_b_returns, method='pearson')
                    msg = 'Correlation = `{:.2f}`\n'.format(corr)

                    # Calculate beta
                    model = OLS(data_a_returns, add_constant(data_b_returns))
                    reg = model.fit()
                    beta_1 = reg.params[1]
                    pvalue_1 = adfuller(reg.resid)[1]
                    model2 = OLS(data_b_returns, add_constant(data_a_returns))
                    reg2 = model2.fit()
                    beta_2 = reg2.params[1]
                    pvalue_2 = adfuller(reg2.resid)[1]

                    if pvalue_1 < pvalue_2:
                        beta = beta_1
                        dollar_size_a = dollar_size
                        dollar_size_b = dollar_size * beta
                        units_a = dollar_size_a/data_a.iloc[-1]['close']
                        units_b = dollar_size_b / data_b.iloc[-1]['close']
                    else:
                        beta = beta_2
                        dollar_size_a = dollar_size * beta
                        dollar_size_b = dollar_size 
                        units_a = dollar_size_a/data_a.iloc[-1]['close']
                        units_b = dollar_size_b / data_b.iloc[-1]['close']

                    if beta > 0:
                        #msg += 'Beta = `{:.2f}` \nFor `${:.2f}` of `{}`, you need `${:.2f}` of `{}`'.format(beta, asset_a, 100*beta, asset_b)
                        msg += 'Beta = `{:.2f}` \nFor `{:.2f}` units of `{}` worth `{:.2f}$`, you need `{:.2f}` units of `{}` worth `{:.2f}$`'.format(beta, units_a, asset_a.upper(), dollar_size_a, units_b, asset_b.upper(), dollar_size_b)
                    else:
